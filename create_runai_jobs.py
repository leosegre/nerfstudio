...
def main(reg_pipline):
    ...
    for scene_name in scene_names:
        for exp_type in exp_types:
            cmd = f"runai submit --pvc=storage:/storage -i leosegre/nerfstudio_reg --name leo-{scene_name}-{exp_type} " \
                  f"-g 1 --large-shm --command -- bash entrypoint.sh python {reg_pipline} " \
