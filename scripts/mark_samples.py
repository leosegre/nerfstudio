...
def sample_method(mask):
    ...
    chosen_indices = random.sample(range(len(nonzero_indices)), k=32000)
...
def mark_samples(input_image_path, input_mask_path, output_image_path):
    ...
    img_masked.putalpha(256)
    img_random = Image.open(input_image_path)
    img_random.putalpha(256)
    ...
    mark_indices(draw_masked, masked_indices, (0, 0, 255))  # Green
