...
def main(data_dir, outputs_dir, scene_names, exp_types, downscale, timestamp=None):
    ...
    for exp in exps:
        ...
        registeration_cmd = default_params_registration + " --output-dir " + exp["outputs_dir"] + \
                            ...
                            + " --data " + exp["inerf_data"] \
                            + " --experiment_name " + exp[
                                "experiment_name"] + "_registration_inerf_no_init --timestamp " + timestamp \
        ...
        gen_inerf_tranform_cmd = "python gen_inerf_transform_file.py " + exp["inerf_data"] \
                                 + f"{data_dir}/{exp['scene_name']}/transforms.json" \
                                 + outputs_dir + exp["experiment_name"] + "_registered/nerfacto/" + timestamp + "/dataparser_transforms.json" \
                                 + scene_seed
        ...
        for i in range(1, 11):
            ...
            exp_stats_path = outputs_dir + exp[
                "experiment_name"] + "_registration_inerf_no_init/nerfacto/" + timestamp + "/stats.json"
    ...
    if not os.path.exists(base_dir):
        os.mkdir(base_dir)
    total_stats_path = base_dir + curr_timestamp + str(random.randint(0, 100)) + "_inerf.json"
