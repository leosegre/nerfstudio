...
def main(data_dir, outputs_dir, scene_names, exp_types, noise_levels, downscale, timestamp=None, repeat_reg=10):
    ...
    default_params = "ns-train nerfacto --viewer.quit-on-train-completion True --max-num-iterations 60000 --nf-first-iter 50000 --pipeline.model.noise-oriented-points True --pipeline.datamanager.train-num-rays-per-batch 1024 " \
    ...
    for exp in exps:
        ...
        export_cmd_reg = "ns-export nf-cameras --load-config " + outputs_dir + exp["experiment_name"] + "_registered/nerfacto/" \
                     ...
                     + " --num_points " + exp["num_points_reg"] + " --downscale_factor " + exp["reg_downscale_factor"]
    ...
    if not os.path.exists(base_dir):
        os.mkdir(base_dir)
    total_stats_path = base_dir + curr_timestamp + str(random.randint(0, 100)) + "_noise.json"
